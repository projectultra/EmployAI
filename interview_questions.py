...
query = (Query('(*)=>[KNN 5 @question_vector $query_vector AS vector_score]').
         sort_by('vector_score').return_fields('vector_score', 'Question',
                                               'Text').dialect(2))
...
def create_query_table(query, queries, encoded_queries):
    ...
    for i, encoded_query in enumerate(encoded_queries):
        result_docs = (client.ft("idx:double_vectors").search(
            query, {
...
def question_answers(role=None, queries, description):
    ...
    messages = [{
        ...
        "system",
        "content":
        '''You are an expert hiring manager and a highly skilled job trainer.
        ...
        "user",
        "content":
        f"Here is the Job description that you have to tailor the questions and answers to: {description}"
    }]
    if role is not None:
        messages.append({
            "role":
            "user",
            "content":
            f"Give me 5 interview questions {role} with the respective answer."
        })
    else:
        messages.append({
            "role":
            "user",
            "content":
            f"Give me 5 interview questions with the respective answer."
        })
...
if __name__ == "__main__":
    ...
    queries = [
        ...
        'Problem Solving and Critical Thinking', 'Learning and Adaptability',
        'Work Ethic and Reliability', 'Leadership and Management'
    ]
